...
import { createOnlineOrder, type MenuCategory, type MenuItem } from "@/app/actions/online-order";
              ...
              return (
                <div key={id} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {itemNotes[id] && <div className="mt-0.5 text-xs text-muted-foreground truncate">Note: {itemNotes[id]}</div>}
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Button size="icon" variant="outline" className="size-8 rounded-full" onClick={() => setQty(id, -1)}><Minus className="size-3.5" /></Button>
                    <span className="w-5 text-center text-sm font-medium tabular-nums">{qty}</span>
                    <Button size="icon" variant="outline" className="size-8 rounded-full" onClick={() => setQty(id, 1)}><Plus className="size-3.5" /></Button>
                  </div>
                  <div className="text-sm font-medium tabular-nums w-20 text-right shrink-0">{formatRp(item.price * qty)}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="fixed inset-x-0 bottom-0 z-20">
          <div className="mx-auto w-full max-w-md border-t bg-background px-5 py-3">
            <Button className="h-12 w-full text-base" onClick={confirm} disabled={submitting || totalQty === 0}>
              {submitting ? "Placing order…" : `Continue to payment · ${formatRp(subtotal)}`}
            </Button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-1 flex-col">
      ...
      <header className="sticky top-0 z-20 border-b bg-background/95 backdrop-blur">
        <div className="px-5 py-3">
          <div className="relative">
            ...
            <Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search menu…" className="h-11 pl-9" />
          </div>
        </div>
          ...
          <div className="flex gap-2 overflow-x-auto px-5 pb-3 no-scrollbar">
            {categories.map((c) => (
              <button
                ...
                className={`shrink-0 rounded-full border px-3.5 py-1.5 text-sm transition-colors ${activeCat === c.name ? "border-primary bg-primary text-primary-foreground" : "hover:bg-accent"}`}
              >
                {c.name}
              </button>
            ))}
      ...
      <div className="flex-1 px-5 py-4 pb-28 space-y-7">
        {categories.length === 0 && <p className="py-10 text-center text-sm text-muted-foreground">No menu available yet.</p>}
        {q ? (
          ...
          categories.map((cat) => (
            <section key={cat.name} ref={(el) => { sectionRefs.current[cat.name] = el; }} className="scroll-mt-40 space-y-3">
        ...
        <div className="fixed inset-x-0 bottom-0 z-20">
          <div className="mx-auto w-full max-w-md border-t bg-background px-5 py-3">
            <Button className="h-12 w-full justify-between text-base" onClick={() => setView("review")}>
              <span className="flex items-center gap-2"><ShoppingBag className="size-5" />{totalQty} item{totalQty > 1 ? "s" : ""}</span>
              <span className="tabular-nums">{formatRp(subtotal)}</span>
            </Button>
          </div>
        </div>
      ...
      <ItemSheet
        ...
        onAdd={(itemQty, note) => {
          ...
          setCart((prev) => ({ ...prev, [id]: (prev[id] ?? 0) + itemQty }));
          setItemNotes((prev) => { const copy = { ...prev }; if (note) copy[id] = note; else delete copy[id]; return copy; });
          setSheetItem(null);
        }}
      />
    </div>
...
function MenuCard({ item, qty, onAdd, onOpen }: { item: MenuItem; qty: number; onAdd: () => void; onOpen: () => void }) {
  return (
    <button onClick={onOpen} className="group flex flex-col overflow-hidden rounded-xl border text-left transition-shadow hover:shadow-sm">
      <div className="relative aspect-square w-full bg-muted">
        ...
        >
          ...
          {qty > 0 && (
            <span className="absolute -top-1.5 -right-1.5 flex min-w-5 items-center justify-center rounded-full bg-foreground px-1 text-[11px] font-semibold text-background">{qty}</span>
          )}
        </span>
      </div>
      <div className="flex flex-1 flex-col gap-0.5 p-2.5">
        <div className="line-clamp-2 text-sm font-medium leading-snug">{item.name}</div>
        <div className="mt-auto text-sm font-semibold tabular-nums">{formatRp(item.price)}</div>
      </div>
    </button>
  );
}

function ItemSheet({ item, onClose, onAdd }: { item: MenuItem | null; onClose: () => void; onAdd: (qty: number, note: string) => void }) {
          ...
          <>
            ...
            <div className="border-t px-5 py-3 flex items-center gap-3">
              <div className="flex items-center gap-2">
                <Button size="icon" variant="outline" className="size-9 rounded-full" onClick={() => setQty((n) => Math.max(1, n - 1))}><Minus className="size-4" /></Button>
                <span className="w-6 text-center text-base font-medium tabular-nums">{qty}</span>
                <Button size="icon" variant="outline" className="size-9 rounded-full" onClick={() => setQty((n) => n + 1)}><Plus className="size-4" /></Button>
              </div>
              <Button className="h-11 flex-1 justify-between text-base" onClick={() => onAdd(qty, note.trim())}>
                <span>Add</span>
                <span className="tabular-nums">{formatRp(item.price * qty)}</span>
              </Button>
            </div>
          </>
        )}
